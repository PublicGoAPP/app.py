...
st.set_page_config(page_title="Public Go Elite v76", layout="wide")
...
if not df_intel.empty:
    for _, row in df_intel.iterrows():
        ...
        if "PETRÓLEO" in raw_cat or "PETROLEO" in raw_cat:
            cat_limpia, bg_color = "PETRÓLEO", color_map["PETRÓLEO"]
        elif "ECONOMÍA" in raw_cat or "ECONOMIA" in raw_cat:
            cat_limpia, bg_color = "ECONOMÍA", color_map["ECONOMÍA"]
        elif "POLÍTICA" in raw_cat or "POLITICA" in raw_cat:
            cat_limpia, bg_color = "POLÍTICA", color_map["POLÍTICA"]
        elif "RELACIONES" in raw_cat:
            cat_limpia, bg_color = "RELACIONES INTERNACIONALES", color_map["RELACIONES INTERNACIONALES"]
        else:
            cat_limpia, bg_color = "GENERAL", color_map["GENERAL"]

        # --- PROCESAMIENTO ULTRA-ROBUSTO DE ANÁLISIS ---
