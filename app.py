...
if not df_intel.empty:
    for _, row in df_intel.iterrows():
        ...
        texto_full = str(row.get("Analisis", "")).replace("SEPARADOR", "").split("CATEGORIA:")[0].strip()
        
        # Búsqueda robusta de la recomendación
        keywords = ["RECOMENDACIÓN EJECUTIVA:", "RECOMENDACIÓN:", "RECOMENDACION:"]
        cuerpo = texto_full
        rec_texto = ""

        for word in keywords:
            if word in texto_full.upper():
                partes = texto_full.upper().split(word, 1)
                cuerpo = texto_full[:len(partes[0])].strip()
                rec_texto = texto_full[len(partes[0]) + len(word):].strip()
                break

        # Limpieza de títulos repetidos en el cuerpo
        cuerpo_limpio = cuerpo.replace("Perspectiva Estratégica:", "").strip()
