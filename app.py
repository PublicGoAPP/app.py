...
st.set_page_config(page_title="Public Go Elite v55.5", layout="wide")
...
def generar_analisis_inteligente(cat, data, alcance):
    ...
    # Prompt optimizado para trabajar menos y ser más preciso
    prompt = f"""
    Eres la Directora de Estrategia de Public Go. Analiza estos hechos de {cat} en Venezuela ({alcance}) para hoy 27 de febrero 2026: {titulares}. 
    
    INSTRUCCIONES:
    1. Realiza un diagnóstico cualitativo profundo.
    2. SOLO si hay cifras relevantes en los titulares, realiza un breve análisis cuantitativo. Si no hay datos numéricos, omite esta parte.
    3. Concluye con una recomendación estratégica.
    """
    try:
...
# 1. Cambio de nombre del botón solicitado
if st.button("🚀 ANÁLISIS INFORMATIVO E INTELIGENCIA"):
    for cat, q in CATEGORIAS.items():
        ...
        if noticias:
            # 2. Inversión de lugares: Noticias al centro (izq) y Diagnóstico a la derecha
            ...
            with col_diag:
                st.write("**🧠 Diagnóstico Public Go**")
